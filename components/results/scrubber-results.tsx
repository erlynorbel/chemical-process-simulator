...
interface ScrubberResultsProps {
  results: {
    input: {
      [key: string]: number
    }
...
export function ScrubberResults({ results }: ScrubberResultsProps) {
  const MW = {
  ...
  const offgasTotal = Object.values(results.offgas).reduce((sum, val) => sum + val, 0)
  const offgasTotalMass = Object.entries(results.offgas).reduce((sum, [comp, val]) => {
    return sum + val * MW[comp as keyof typeof MW]
  }, 0)
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Scrubber Unit Balance</CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            ...
            {Object.entries(results.offgas).map(([component, value]) => (
              <TableRow key={`offgas-${component}`}>
                <TableCell>Off-Gas</TableCell>
                <TableCell>{component}</TableCell>
                <TableCell className="text-right"></TableCell>
                <TableCell className="text-right"></TableCell>
                <TableCell className="text-right">{value.toFixed(2)}</TableCell>
            ...
            <TableRow className="font-medium">
              ...
              <TableCell className="text-right"></TableCell>
              <TableCell className="text-right"></TableCell>
              <TableCell className="text-right">{offgasTotal.toFixed(2)}</TableCell>
              <TableCell className="text-right">{offgasTotalMass.toFixed(2)}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell colSpan={6} className="border-t border-gray-200"></TableCell>
            </TableRow>
            ...
            <TableRow className="font-medium">
              ...
              <TableCell className="text-right"></TableCell>
              <TableCell className="text-right"></TableCell>
              <TableCell className="text-right">{(offgasTotal + liquidTotal + lossesTotal).toFixed(2)}</TableCell>
              <TableCell className="text-right">
                {(offgasTotalMass + liquidTotalMass + lossesTotalMass).toFixed(2)}
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
